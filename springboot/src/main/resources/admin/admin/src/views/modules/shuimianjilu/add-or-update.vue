	...
	<div class="addEdit-block" :style='{"padding":"0 30px","margin":"0 auto","color":"#666","borderRadius":"4px","background":"none","width":"96%","fontSize":"16px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="记录时间" prop="jilushijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.jilushijian" label="记录时间" prop="jilushijian">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="睡眠记录" prop="shuimianjilu">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.shuimianjilu" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.shuimianjilu" label="睡眠记录" prop="shuimianjilu">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.shuimianjilu"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='shuimianjilu'){
							this.ruleForm.shuimianjilu = obj[o];
							this.ro.shuimianjilu = true;
							continue;
						}
						if(o=='jilushijian'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.shuimianjilu = this.ruleForm.shuimianjilu.replace(reg,'../../../springbooty7qe733u/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `shuimianjilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shuimianjiluCrossAddOrUpdateFlag = false;
											this.parent.search();
