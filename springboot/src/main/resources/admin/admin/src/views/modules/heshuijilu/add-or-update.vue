	...
	<div class="addEdit-block" :style='{"padding":"0 30px","margin":"0 auto","color":"#666","borderRadius":"4px","background":"none","width":"96%","fontSize":"16px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="喝水记录" prop="heshuijilu">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.heshuijilu" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.heshuijilu" label="喝水记录" prop="heshuijilu">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.heshuijilu"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='heshuijilu'){
							this.ruleForm.heshuijilu = obj[o];
							this.ro.heshuijilu = true;
							continue;
						}
						if(o=='jilushijian'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.heshuijilu = this.ruleForm.heshuijilu.replace(reg,'../../../springbooty7qe733u/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `heshuijilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.heshuijiluCrossAddOrUpdateFlag = false;
											this.parent.search();
