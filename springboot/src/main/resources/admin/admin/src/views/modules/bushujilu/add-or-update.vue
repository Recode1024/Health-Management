	...
	<div class="addEdit-block" :style='{"padding":"0 30px","margin":"0 auto","color":"#666","borderRadius":"4px","background":"none","width":"96%","fontSize":"16px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="步数记录" prop="bushujilu">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.bushujilu" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.bushujilu" label="步数记录" prop="bushujilu">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.bushujilu"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.bushujilu = this.ruleForm.bushujilu.replace(reg,'../../../springbooty7qe733u/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `bushujilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.bushujiluCrossAddOrUpdateFlag = false;
											this.parent.search();
